...
export default function ServerMetricsScreen({ route, navigation }: Props) {
  ...
  const loadMetrics = async () => {
    ...
    try {
      ...
      setCpuData(
        (cpuMetrics.time_series?.cpu?.values ?? []).map(([, v]) => ({
          value: parseFloat(v),
        })),
      );
      setDiskReadData(
        (diskMetrics.time_series?.["disk.0.bandwidth.read"]?.values ?? []).map(
          ([, v]) => ({ value: parseFloat(v) / 1024 }),
        ),
      );
      setDiskWriteData(
        (diskMetrics.time_series?.["disk.0.bandwidth.write"]?.values ?? []).map(
          ([, v]) => ({ value: parseFloat(v) / 1024 }),
        ),
      );
      setNetInData(
        (netMetrics.time_series?.["network.0.bandwidth.in"]?.values ?? []).map(
          ([, v]) => ({ value: parseFloat(v) / 1024 }),
        ),
      );
      setNetOutData(
        (netMetrics.time_series?.["network.0.bandwidth.out"]?.values ?? []).map(
          ([, v]) => ({ value: parseFloat(v) / 1024 }),
        ),
      );
    } catch {}
    ...
    <View style={styles.chart}>
      <View style={styles.chartHeader}>
        ...
        <Text style={styles.chartCurrent}>
          {display} <Text style={styles.chartUnit}>{unit}</Text>
        </Text>
      </View>
      <LineChart
        data={data}
