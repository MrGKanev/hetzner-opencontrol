...
export default function ServerDetailScreen({ route, navigation }: Props) {
  ...
  const runAction = async (label: string, fn: () => Promise<any>) => {
    ...
    Alert.alert(
      label,
      `Are you sure you want to ${label.toLowerCase()} this server?`,
      [
  ...
  const showActions = () => {
    ...
    if (Platform.OS === "ios") {
      showActionSheet({
        title: server.name,
        options,
        onSelect: (i) => handleActionIndex(i, options, server),
      });
    } else {
  ...
  const handleActionIndex = (
    ...
    switch (options[index]!.label) {
      ...
      case "Graceful Shutdown":
        runAction("Shutdown", () => shutdownServer(s.id));
        break;
      case "Reboot":
        runAction("Reboot", () => rebootServer(s.id));
        break;
      case "Hard Reset":
        runAction("Hard Reset", () => resetServer(s.id));
        break;
      case "Power On":
        runAction("Power On", () => powerOnServer(s.id));
        break;
      case "Open Console":
      ...
      case "Attach ISO":
        ...
        break;
      case "Detach ISO":
        runAction("Detach ISO", () => detachIso(s.id));
        break;
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Text
            style={[styles.tabText, tab === "overview" && styles.tabTextActive]}
          >
            Overview
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate("ServerMetrics", { serverId: server.id })
          }
        >
        ...
        >
          <Text
            style={[
              styles.tabText,
              tab === "activities" && styles.tabTextActive,
            ]}
          >
            Activity
          </Text>
        ...
        {tab === "overview" && (
          <>
            ...
            <Section title="General Information" colors={colors}>
              ...
              <InfoRow
                label="Created"
                value={new Date(server.created).toLocaleString()}
                colors={colors}
              />
              <InfoRow
                label="Type"
              ...
              <InfoRow
                label="Cores"
                value={String(server.server_type.cores)}
                colors={colors}
              />
              <InfoRow
                label="Memory"
                value={`${server.server_type.memory} GB`}
                colors={colors}
              />
              <InfoRow
                label="Disk"
                value={`${server.server_type.disk} GB`}
                colors={colors}
              />
              <InfoRow
                label="Architecture"
                value={server.server_type.architecture}
                colors={colors}
              />
            </Section>
            ...
            <Section title="Location" colors={colors}>
              <InfoRow
                label="Datacenter"
                value={server.datacenter.name}
                colors={colors}
              />
              <InfoRow
                label="City"
                value={server.datacenter.location.city}
                colors={colors}
              />
              <InfoRow
                label="Country"
                value={server.datacenter.location.country}
                colors={colors}
              />
            </Section>
            ...
            <Section title="Protection" colors={colors}>
              <InfoRow
                label="Delete"
                value={server.protection.delete ? "Enabled" : "Disabled"}
                colors={colors}
              />
              <InfoRow
                label="Rebuild"
                value={server.protection.rebuild ? "Enabled" : "Disabled"}
                colors={colors}
              />
            </Section>
          </>
        ...
        {actions.map((action, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <Text style={[styles.activityIconText, { color: statusColor }]}>
                  {icon}
                </Text>
              </View>
              ...
              {action.status === "running" && (
                <Text
                  style={[styles.activityProgress, { color: colors.primary }]}
                >
                  {action.progress}%
                </Text>
              )}
