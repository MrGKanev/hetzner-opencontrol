...
const actions = [{ label: "Delete", icon: "🗑", destructive: true }];
  ...
  const handleAction = async (index: number, group: PlacementGroup) => {
    if (actions[index]!.label === "Delete") {
      if (group.servers.length > 0) {
        Alert.alert(
          "Cannot Delete",
          "Remove all servers from this placement group before deleting it.",
        );
        return;
      }
      Alert.alert(
  ...
  const renderItem = ({ item }: { item: PlacementGroup }) => (
    ...
    >
      ...
      <View
        style={[
          styles.badge,
          item.servers.length > 0 ? styles.badgeActive : styles.badgeEmpty,
        ]}
      >
        <Text
          style={[
            ...
            {
              color:
                item.servers.length > 0 ? colors.primary : colors.textMuted,
            },
          ]}
        >
