...
export default function PricingCalculatorScreen() {
  ...
  const [selectedServerType, setSelectedServerType] =
    useState<ServerTypePricing | null>(null);
  ...
  const selectedPricing = useMemo(() => {
    ...
    return (
      selectedServerType.pricings.find(
        (p) => p.location.name === selectedLocation,
      ) ?? null
    );
  ...
  const estimate = useMemo(() => {
    ...
    const backupMonthly =
      includeBackup && selectedPricing
        ? serverMonthly * (parseFloat(pricing.server_backup.percentage) / 100)
        : 0;

    const gb = parseInt(volumeGb, 10) || 0;
    const volumeMonthly =
      gb > 0 ? gb * parseFloat(pricing.volume.price_per_gb_month.gross) : 0;

    const floatingMonthly =
      floatingIps > 0
        ? floatingIps * parseFloat(pricing.floating_ip.price_monthly.gross)
        : 0;

    const total =
      serverMonthly + backupMonthly + volumeMonthly + floatingMonthly;
      ...
      >
        ...
        {serverTypesForLocation.map((st) => {
          ...
          return (
            ...
            >
              ...
              >
                {parseFloat(p.price_monthly.gross).toFixed(2)}{" "}
                {pricing!.currency}/mo
              </Text>
              {active && (
                <Icon
        ...
        <View style={styles.card}>
          ...
          >
            <Icon
              name={
                includeBackup ? "checkbox-marked" : "checkbox-blank-outline"
              }
              size={20}
          ...
          <View style={styles.extraRow}>
            ...
            <View style={styles.extraLabel}>
              ...
              {pricing && (
                <Text style={styles.extraSub}>
                  {parseFloat(pricing.volume.price_per_gb_month.gross).toFixed(
                    4,
                  )}{" "}
                  {pricing.currency}/GB/mo
                </Text>
              )}
            </View>
          ...
          <View style={styles.extraRow}>
            ...
            <View style={styles.extraLabel}>
              ...
              {pricing && (
                <Text style={styles.extraSub}>
                  {parseFloat(pricing.floating_ip.price_monthly.gross).toFixed(
                    2,
                  )}{" "}
                  {pricing.currency}/IP/mo
                </Text>
              )}
            </View>
            ...
            <View style={styles.card}>
              {estimate.server > 0 && (
                <View style={styles.lineRow}>
                  ...
                  <Text style={styles.lineValue}>
                    {fmt(estimate.server, estimate.currency)}
                  </Text>
                </View>
              )}
              {estimate.backup > 0 && (
                <View style={styles.lineRow}>
                  ...
                  <Text style={styles.lineValue}>
                    {fmt(estimate.backup, estimate.currency)}
                  </Text>
                </View>
              )}
              {estimate.volume > 0 && (
                <View style={styles.lineRow}>
                  ...
                  <Text style={styles.lineValue}>
                    {fmt(estimate.volume, estimate.currency)}
                  </Text>
                </View>
              )}
              {estimate.floating > 0 && (
                <View style={styles.lineRow}>
                  ...
                  <Text style={styles.lineValue}>
                    {fmt(estimate.floating, estimate.currency)}
                  </Text>
                </View>
              )}
              <View style={[styles.lineRow, styles.totalRow]}>
                ...
                <Text style={styles.totalValue}>
                  {fmt(estimate.total, estimate.currency)}
                </Text>
              </View>
            </View>
