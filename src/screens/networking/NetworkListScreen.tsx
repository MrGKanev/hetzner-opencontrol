        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.row}
              onPress={() =>
                navigation.navigate("NetworkDetail", { networkId: item.id })
              }
              activeOpacity={0.7}
            >
              <View style={styles.rowLeft}>
                ...
                <Text style={styles.netMeta}>
                  {item.ip_range} · {item.subnets.length} subnet
                  {item.subnets.length !== 1 ? "s" : ""} · {item.servers.length}{" "}
                  server{item.servers.length !== 1 ? "s" : ""}
                </Text>
              </View>
              <TouchableOpacity
                onPress={() =>
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No private networks found</Text>
          }
        />
      )}
