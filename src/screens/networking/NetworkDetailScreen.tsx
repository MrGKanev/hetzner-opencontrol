    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Section title="Overview" colors={colors}>
          ...
          <InfoRow
            label="Servers"
            value={String(network.servers.length)}
            colors={colors}
          />
          <InfoRow
            label="Load Balancers"
            value={String(network.load_balancers.length)}
            colors={colors}
          />
          <InfoRow
            label="Delete Protection"
            value={network.protection.delete ? "Enabled" : "Disabled"}
            colors={colors}
          />
        </Section>
        ...
        <View style={styles.section}>
          ...
          {network.subnets.length === 0 ? (
            ...
            network.subnets.map((subnet, i) => (
              <SubnetRow key={i} subnet={subnet} colors={colors} />
            ))
          )}
        </View>
