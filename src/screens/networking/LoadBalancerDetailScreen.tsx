    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Section title="Overview" colors={colors}>
          <InfoRow
            label="Type"
            value={lb.load_balancer_type.name}
            colors={colors}
          />
          <InfoRow
            label="Algorithm"
          ...
          <InfoRow
            label="Location"
            value={`${lb.location.city} (${lb.location.name})`}
            colors={colors}
          />
          {lb.public_net.enabled && (
            <>
              <InfoRow
                label="IPv4"
                value={lb.public_net.ipv4.ip}
                mono
                colors={colors}
              />
              <InfoRow
                label="IPv6"
                value={lb.public_net.ipv6.ip}
                mono
                colors={colors}
              />
            </>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            Services ({lb.services.length})
          </Text>
          {lb.services.length === 0 ? (
            <EmptyCard text="No services configured" colors={colors} />
          ) : (
            lb.services.map((svc, i) => (
              <ServiceRow key={i} service={svc} colors={colors} />
            ))
          )}
        </View>
        ...
        <View style={styles.section}>
          ...
          {lb.targets.length === 0 ? (
            ...
            lb.targets.map((target, i) => (
              <TargetRow key={i} target={target} colors={colors} />
            ))
          )}
        </View>
      </ScrollView>
    ...
    <View style={styles.card}>
      ...
      <View style={styles.healthRow}>
        ...
        <Text style={styles.healthValue}>
          {service.health_check.protocol.toUpperCase()}:
          {service.health_check.port} · every {service.health_check.interval}s ·
          timeout {service.health_check.timeout}s
        </Text>
      </View>
    </View>
  );
...
function TargetRow({
  ...
  const icon =
    target.type === "server" ? "🖥" : target.type === "ip" ? "🌐" : "🏷";
  return (
